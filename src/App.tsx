...
function ProfilePage({ profile, currentId, posts, following, followers, onFriendAction, onMessage, onBlock, onEdit, onSettings, onModerate, onLogout, online, onReset }: { profile: Profile; currentId: string; posts: ReturnType<typeof useSocialApp>['posts']; following: string[]; followers: string[]; onFriendAction: (id: string) => void; onMessage: (id: string) => void; onBlock: (id: string) => void; onEdit: () => void; onSettings: () => void; onModerate: () => void; onLogout: () => void; online: boolean; onReset: () => void }) {
  ...
  return <div className="page-stack"><section className="card profile-card">
    ...
    {isMine && <div className="profile-menu">{profile.isAdmin && <button onClick={onModerate}><ShieldCheck /> Moderatie <ChevronRight /></button>}<button onClick={onSettings}><Settings /> Instellingen <ChevronRight /></button>{!online && <button onClick={onReset}><MoreHorizontal /> Demo herstellen <ChevronRight /></button>}{online && <button className="danger-text" onClick={onLogout}><LogOut /> Uitloggen <ChevronRight /></button>}</div>}
