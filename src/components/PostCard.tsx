...
import { Ban, CircleAlert, Ellipsis, Flag, Flame, Forward, Frown, Globe2, Heart, Laugh, LoaderCircle, Lock, MessageCircle, Pencil, ThumbsUp, Trash2, UserRound, Users, Vote } from 'lucide-react'
...
export function PostCard({ post, allPosts, profiles, currentUserId, busy, onReaction, onVote, onEdit, onDelete, onOpenComments, onShareProfile, onReport, onBlock, onToast }: {
  ...
  const [pollVoters, setPollVoters] = useState(false)
  ...
  return <article className="card post-card" id={`post-${post.id}`}>
    ...
    {pollVoters && post.poll && <Modal title="Wie hebben gestemd?" onClose={() => setPollVoters(false)}><div className="modal-body poll-voter-list">{post.poll.options.filter((option) => option.voterIds.length > 0).map((option) => <section className="poll-voter-group" key={option.id}><h3><span>{option.text}</span><strong>{option.voterIds.length}</strong></h3>{option.voterIds.map((voterId) => { const voter = profiles.find((person) => person.id === voterId); return voter ? <div className="poll-voter" key={voter.id}><Avatar profile={voter} size={36} /><div><Name profile={voter} /><small>@{voter.username}</small></div></div> : <div className="poll-voter unknown-voter" key={voterId}><span className="avatar avatar-fallback" style={{ width: 36, height: 36 }}><UserRound size={17} /></span><div><strong>Boekoe-gebruiker</strong><small>Stemmer</small></div></div>})}</section>)}{totalVotes === 0 && <p className="muted">Er is nog niet gestemd.</p>}</div></Modal>}
